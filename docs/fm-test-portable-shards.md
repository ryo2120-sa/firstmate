...
The single longest script, `tests/fm-watch-triage.test.sh` at 630058 ms, is the floor for any shard count.
...
| Herdr | family-run step `timeout-minutes: 20`; job `timeout-minutes: 75` backstop | Healthy runs finished around 7 minutes before this lane gained `fm-backend-herdr-focus-flash-e2e`, which measures about 2 minutes against a real lab locally. Two fixtures in `tests/fm-backend-herdr-presentation-e2e.test.sh` hold the shared session presentation lock until the contended spawn returns, but inject a small `FM_BACKEND_HERDR_PRESENTATION_LOCK_WAIT_ATTEMPTS` so they prove the flat-fallback contract without paying the full production wait budget. Budget about 9 minutes healthy, so the step bound is still the hang tripwire (cleanup and timing artifacts still upload) while the job cap stays a last-resort backstop. Refresh this figure from the lane's uploaded timing artifact. |

`fm_backend_herdr_presentation_lock_wait_attempts` (`bin/backends/herdr.sh`) is the single shared wait budget for every blocking waiter on that lock - spawn, spawn abort cleanup, teardown's preflight, and pane kill; [herdr-backend.md](herdr-backend.md) owns that behavior.
