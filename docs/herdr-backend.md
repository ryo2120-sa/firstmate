...
- A spawn waits up to about 70 seconds for that shared presentation lock before it warns and continues flat or refuses, because a peer can legitimately hold it across a bounded 60-second worktree-settle wait; `bin/fm-spawn.sh` owns that budget, the two numbers that must stay in sync, and the unbounded-fetch case in which a peer can still outlast the wait. The same wait runs on the spawn abort path, so a failing or interrupted spawn can take that long to exit.
- Teardown's preflight and the pane kill still give up on the same lock after about 5 seconds, so a teardown that overlaps a peer's legitimate hold can still refuse with nothing changed and a kill can still decline the pane close. That exposure predates the spawn-side budget; unifying the three budgets is a candidate for separate follow-up work.
