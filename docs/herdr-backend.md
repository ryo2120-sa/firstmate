...
- Every blocking waiter on the shared session presentation lock - spawn, spawn abort cleanup, teardown's preflight, and pane kill - waits up to about 70 seconds before it warns and continues flat, refuses, or defers the pane close, because a peer can legitimately hold the lock across a bounded 60-second worktree-settle wait. `fm_backend_herdr_presentation_lock_wait_attempts` in `bin/backends/herdr.sh` is the single owner of that shared budget and the two numbers that must stay in sync, and `bin/fm-spawn.sh`'s acquire site owns the unbounded-fetch case in which a peer can still outlast the wait. The spawn abort path deliberately reuses the same budget rather than a shorter one, so a failing or interrupted spawn can take just as long to exit as a fresh spawn waits to acquire the lock.
