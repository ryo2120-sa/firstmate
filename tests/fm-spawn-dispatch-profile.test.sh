...
test_pi_threads_model_and_max_effort() {
  ...
  assert_contains "$launch" "FM_PI_HARNESS=pi" \
    "pi launch lost its canonical harness marker"
  assert_contains "$launch" "PI_CLAUDE_CODE_PROVIDER_IDLE_TIMEOUT_MS=1800000" \
    "pi launch lost the fixed Claude provider idle timeout"
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed" \
    "pi-signed launch lost its canonical harness marker"
  assert_contains "$launch" "PI_CLAUDE_CODE_PROVIDER_IDLE_TIMEOUT_MS=1800000" \
    "pi-signed launch lost the fixed Claude provider idle timeout"
  assert_contains "$launch" "'$FAKEBIN_DIR/pi-signed' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed" \
    "pi-signed secondmate lost its canonical harness marker"
  assert_contains "$launch" "PI_CLAUDE_CODE_PROVIDER_IDLE_TIMEOUT_MS=1800000" \
    "pi-signed secondmate lost the fixed Claude provider idle timeout"
  assert_contains "$launch" "'$FAKEBIN_DIR/pi-signed' --tui-mode regular -e '$sm/.pi/extensions/fm-primary-turnend-guard.ts' -e '$sm/.pi/extensions/fm-primary-pi-watch.ts'" \
